...
import com.erian.microgrid.webapp.config.AppConfig;
import com.erian.microgrid.webapp.config.DataJpaConfig;
import com.erian.microgrid.webapp.config.DataSourceConfig;
import com.erian.microgrid.webapp.config.JpaConfig;
...
public class DeviceServiceTest {
    ...
    public void setUp() {
        ...
        device = deviceRepository.save(Fixtures.createDevice("My first device", "description of my first device"));
    ...
    public void testSaveDevice() {
        ...
        Page<DeviceDetails> allDevices = deviceService.searchDevicesByCriteria("", null, new PageRequest(0, 10));
        assertTrue(allDevices.getTotalElements() == 2);

        Page<DeviceDetails> devices = deviceService.searchDevicesByCriteria("first", Device.Bus.AC, new PageRequest(0, 10));
        assertTrue(devices.getTotalPages() == 1);
        assertTrue(!devices.getContent().isEmpty());
        assertTrue(Objects.equals(devices.getContent().get(0).getId(), device.getId()));

//        DeviceForm updatingForm = new DeviceForm();
//        updatingForm.setName("updating name");
//        updatingForm.setDescription("updating description");
//        DeviceDetails updatedDetails = deviceService.updateDevice(device.getId(), updatingForm);
